...
def register_user(session: requests.Session, username: str, password: str, email: str) -> Dict[str, Any]:
    ...
    try:
        response = session.post(ENDPOINTS["register"], json=data, headers=headers)
...
def login_user(session: requests.Session, username: str, password: str, email: str) -> Dict[str, Any]:
    ...
    try:
        response = session.post(ENDPOINTS["login"], json=data, headers=headers)
        print_response(response)
...
def get_profile(session: requests.Session) -> Dict[str, Any]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {"error": f"Failed to get profile with status {response.status_code}: {response.text}"}
    except requests.RequestException as e:
...
def test_auth_endpoint(session: requests.Session) -> Dict[str, Any]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {"error": f"Failed auth test with status {response.status_code}: {response.text}"}
    except requests.RequestException as e:
        return {"error": f"Request error: {str(e)}"}
...
def refresh_token_func(session: requests.Session, refresh_token: str) -> Dict[str, Any]:
    ...
    try:
        response = session.post(ENDPOINTS["refresh"], json=data, headers=headers)
        print_response(response)
...
def logout(session: requests.Session, refresh_token: str) -> Dict[str, Any]:
    ...
    try:
        response = session.post(ENDPOINTS["logout"], json=data, headers=headers)
        print_response(response)
...
def test_rate_limiting(endpoint: Optional[str] = None, attempts: int = 15) -> Dict[str, Any]:
    ...
    login_data = {"email": f"test_{random_string()}@example.com", "password": "Test1234!"}

    for i in range(1, attempts + 1):
        try:
            # Send request with proper data
            response = session.post(endpoint, json=login_data, timeout=TIMEOUT)
            ...
            results.append({"request_num": i, "error": str(e)})
    ...
    remaining_values = [
        int(str(result["remaining"])) for result in results if "remaining" in result and result["remaining"] != "N/A"
    ]
    ...
    details = {"requests": results, "rate_limited": rate_limited, "decreasing_quota": decreasing}

    return {"success": success, "error": None if success else "Rate limiting not detected", "details": details}


def test_email_verification(session: requests.Session, username: str, password: str, email: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if security_check_passed:
            ...
            return {
                "success": False,
                "error": "System accepted invalid verification token or returned a suspicious response",
            }
    except requests.RequestException as e:
...
def run_basic_tests() -> Dict[str, Any]:
    ...
    results["email_verification"] = test_email_verification(
        new_session, email_verification_username, password, email_verification_email
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--use-basic-profile", action="store_true", help="Use the basic profile endpoint instead of the main one"
    )
...
def main():
    ...
    if args.rate_limit:
        ...
        if "details" in results:
            ...
            for request in results["details"]["requests"]:
                print(
                    f"Request {request['request_num']}: Status {request['status']}, Remaining: {request['remaining']}, Reset: {request['reset']}"
                )
    ...
    print(
        f"{'✅' if results.get('direct_auth') else '❌'} Direct Authentication: {results.get('direct_auth', 'Not tested')}"
    )
    if "direct_auth" in error_details:
        print(f"   Error: {error_details['direct_auth']}")
    print(
        f"{'✅' if results.get('profile_access') else '❌'} Protected Resource Access: {results.get('profile_access', 'Not tested')}"
    )
