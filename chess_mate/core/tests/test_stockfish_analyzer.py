...
class TestStockfishAnalyzer(TransactionTestCase):
    ...
    def test_score_conversion_edge_cases(self):
        ...
        test_cases = [
            ...
            {"name": "null_score", "score": None, "expected": 0.0},
        ]
    ...
    def test_time_management(self):
        ...
        for case in test_cases:
            with self.subTest(case=case["name"]):
                metrics = self.analyzer._calculate_time_metrics(
                    case["time_spent"], case["total_time"], case["increment"]
                )

                self.assertEqual(
                    metrics["time_pressure"],
                    case["expected"]["time_pressure"],
                    f"Time pressure mismatch for {case['name']}",
                )
                self.assertEqual(
                    metrics["critical_time"],
                    case["expected"]["critical_time"],
