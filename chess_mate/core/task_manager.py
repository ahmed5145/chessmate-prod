...
class TaskManager:
            ...
            if self.redis_client is not None:
                ...
                if task_data:
                    try:
                        ...
                        if task_info:
                            mem_updated = str(task_info.get("updated_at", ""))
                            redis_updated = str(redis_info.get("updated_at", ""))
                            if mem_updated >= redis_updated:
                                task_info = {**redis_info, **task_info}
                            else:
    ...
    def get_task_status(
        ...
        elif str(status).upper() == TASK_STATUS_PENDING and (
            ...
            for pool in (inspect.active(), inspect.reserved(), inspect.scheduled()):
                ...
                for worker_tasks in pool.values():
                    for entry in worker_tasks or []:
                        request = (
                            entry.get("request") if isinstance(entry, dict) else None
                        )
    ...
    def _reset_stuck_game_status(self, game_id: Union[int, str]) -> None:
        try:
            ...
            logger.debug(
                "Could not reset analysis_status for game %s: %s", game_id, exc
            )
    ...
    def get_active_tasks_for_game(self, game_id: Union[int, str]) -> List[str]:
        ...
        try:
            ...
            if self._is_stale_queued_task(task_id, task_info):
                logger.warning(
                    "Abandoning stale queued task %s for game %s", task_id, game_id
                )
                self.abandon_stale_queued_task(task_id, game_id)
                return []
