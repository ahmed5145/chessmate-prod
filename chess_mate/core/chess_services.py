        ...
        for attempt in range(cls.MAX_RETRIES):
            try:
                ...
                if attempt < cls.MAX_RETRIES - 1:
                    logger.warning(f"Request failed ({str(e)}), attempt {attempt + 1}/{cls.MAX_RETRIES}. Waiting {retry_delay}s")
        ...
        try:
            ...
            for archive_url in reversed(archives):
                try:
                    ...
                    matching_games = [g for g in games if g.get('time_class') == game_type] if game_type != 'all' else games
                    ...
                    for game in matching_games:
                        ...
                        if game_id in unique_games or Game.objects.filter(game_id=game_id, platform='chess.com', user=user).exists():
                            skipped_count += 1
                            continue
    ...
    def save_game(self, game_data: Dict[str, Any], username: str, user: User) -> Optional[Game]:
        ...
        try:
            ...
            try:
                ...
                if not profile.chesscom_username:
...
class LichessService:
    ...
    def fetch_games(username: str, user: User, game_type: str = "rapid", limit: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            message = (f"Successfully imported {games_saved} new {game_type} games "
...
def save_game(game: Dict[str, Any], username: str, user: User) -> Optional[Game]:
    ...
    try:
        ...
        if not (is_white or is_black):
            logger.warning(f"Game does not belong to requested player. Input: {input_username}, White: {white_username}, Black: {black_username}")
            return None
            
        # Get game URL and check for duplicates
        game_url = game.get("url")
        if not game_url or Game.objects.filter(game_url=game_url).exists():
            logger.info(f"Game with URL {game_url} already exists. Skipping.")
            return None
        ...
        logger.info(f"Successfully saved game {game_url} for user {input_username} (played as {'white' if is_white else 'black'} with rating {player_rating})")
        return saved_game
