// Auto-synced from core/eco_codes.py ECO_OPENINGS
...
export const getEcoOpeningName = (ecoCode) => {
  if (!ecoCode) return null;
  const code = String(ecoCode).trim().toUpperCase();
  if (ECO_OPENINGS[code]) return ECO_OPENINGS[code];
  if (code.length === 3 && /^[A-E][0-9]{2}$/.test(code)) {
    const letter = code[0];
    const num = parseInt(code.slice(1), 10);
    for (let n = num - 1; n >= 0; n -= 1) {
      const parent = letter + String(n).padStart(2, '0');
      if (ECO_OPENINGS[parent]) return ECO_OPENINGS[parent];
    }
  }
  return null;
};
